// ===== HABIT TRACKER CLASS WITH FIREBASE =====
let tracker = null;
...
class HabitTracker {
    ...
    subscribeToData() {
        ...
        // Subscribe to habits
        this.unsubscribeHabits = userRef.collection('habits')
        ...
        this.unsubscribeHistory = userRef.collection('history')
            .onSnapshot((snapshot) => {
                this.history = {};
    ...
    async addHabit(name, category) {
        ...
        try {
            await this.getUserRef().collection('habits').add({
                name: name.trim(),
                category,
    ...
    async deleteHabit(id) {
        ...
        try {
            await this.getUserRef().collection('habits').doc(id).delete();

            // Clean up history entries
            const batch = db.batch();
            Object.keys(this.history).forEach(date => {
                if (this.history[date][id] !== undefined) {
                    const historyRef = this.getUserRef().collection('history').doc(date);
    ...
    async toggleHabit(id) {
        ...
        const isCompleted = this.history[today]?.[id] || false;
        const newValue = !isCompleted;

        this.showSyncStatus('syncing');

        try {
            await this.getUserRef().collection('history').doc(today).set({
                [id]: newValue
            }, { merge: true });
    ...
    async updateStreak(id) {
        ...
        try {
            await this.getUserRef().collection('habits').doc(id).update({
                streak: streak
            });
        } catch (error) {
    ...
    getStats() {
        ...
        return { currentStreak, longestStreak, completionRate, totalCompleted };
    }
